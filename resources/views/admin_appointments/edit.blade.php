            ...
            <div class="col-lg-12">
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body p-4">
                        <form id="availabilityForm" action="{{ route('admin_appointments.updateBatch', $batch_id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="selection_type" id="selectionType" value="{{ $selectionType }}">
                            <input type="hidden" name="schedule_data" id="scheduleData" value="{{ json_encode($scheduleData) }}">
                                    ...
                                    <div class="calendar-section h-100">
                                        <label class="form-label fw-bold mb-3 d-flex justify-content-between align-items-center text-secondary">
                                            ...
                                            <span class="badge badge-primary-custom" id="selectionModeBadge" style="font-size: 0.7rem;">
                                                {{ $selectionType == 'range' ? __('Modo Rango') : ($selectionType == 'custom' ? __('Modo Individual') : __('Días Laborables')) }}
                                            </span>
                                        </label>
                            ...
                            <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-4 pt-3 border-top">
                                ...
                                <button type="submit" id="submitBtn" class="btn btn-primary-custom px-4 shadow-sm">
                                    ...
                                    <select class="form-select" id="generalStartTime">
                                        @for ($hour = 6; $hour <= 22; $hour++)
                                            @for ($minute = 0; $minute < 60; $minute += 30)
                                                ...
                                                <option value="{{ $time }}" {{ isset($scheduleData[0]['start_time']) && $scheduleData[0]['start_time'] == $time ? 'selected' : ($hour == 9 && $minute == 0 && !isset($scheduleData[0]['start_time']) ? 'selected' : '') }}>
                                                    {{ $time }}
                                                </option>
                                            @endfor
                                        @endfor
                                ...
                                <div class="input-group">
                                    ...
                                    <select class="form-select" id="generalEndTime">
                                        @for ($hour = 6; $hour <= 22; $hour++)
                                            @for ($minute = 0; $minute < 60; $minute += 30)
                                                @php $time = sprintf('%02d:%02d', $hour, $minute); @endphp
                                                <option value="{{ $time }}" {{ isset($scheduleData[0]['end_time']) && $scheduleData[0]['end_time'] == $time ? 'selected' : ($hour == 18 && $minute == 0 && !isset($scheduleData[0]['end_time']) ? 'selected' : '') }}>
                                                    {{ $time }}
                                                </option>
                                            @endfor
                                        @endfor
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const initialDates = @json($datesList);
            const initialScheduleData = @json($scheduleData);

            // State
            let currentMode = initialSelectionType;
            ...
            */
            ...
            document.getElementById('selectCustomBtn').addEventListener('click', function(e) {
                e.preventDefault();
                // Solo modo individual, no hay cambio de modo
            });
            ...
            */
            ...
            // Modo individual (custom) - único modo activo
            /*
            ...
            */
            ...
            document.getElementById('availabilityForm').addEventListener('submit', function(e) {
                ...
                try {
                    currentScheduleData = JSON.parse(scheduleDataInput.value);
                } catch (err) {
                    currentScheduleData = [];
                }

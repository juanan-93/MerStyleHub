                            ...
                            <div class="appointment-list d-flex flex-column gap-1 overflow-auto" style="max-height: 90px;">
                                @foreach($daySlots as $slot)
                                    ...
                                    @if($slot['is_user_appointment'] || $slot['status'] === 'available')
                                        <div class="appointment-card {{ $statusClass }} {{ $isPastDate ? 'past-date' : '' }} {{ $isClickable && !$isPastDate ? 'slot-clickable' : '' }} {{ $isPastDate && $slot['is_user_appointment'] ? 'past-clickable' : '' }} small d-flex align-items-center justify-content-between px-2" 
                                             data-slot='@json($slot)'
                                             data-date="{{ $cellDate }}"
                                ...
                                <td class="week-cell p-1 {{ $isToday ? 'today-cell' : '' }} {{ $isPastDate ? 'past-cell' : '' }}" 
                                    ...
                                    <div class="week-appointments d-flex flex-column gap-1">
                                        @foreach($hourSlots as $slot)
                                            ...
                                            @endphp
                                            @if($slot['is_user_appointment'] || $slot['status'] === 'available')
                                                <div class="appointment-card week-appointment {{ $statusClass }} {{ $isPastDate ? 'past-date' : '' }} {{ $isClickable && !$isPastDate ? 'slot-clickable' : '' }} {{ $isPastDate && $slot['is_user_appointment'] ? 'past-clickable' : '' }} small" 
                                                     data-slot='@json($slot)'
                                                     data-date="{{ $cellDate }}"
                                                     data-status="{{ $slot['status'] }}"
                                                     data-is-past="{{ $isPastDate ? 'true' : 'false' }}"
            ...
            <div class="modal-body pt-3">
                ...
                <div id="availableSlotInfo" style="display: none;">
                    <div class="text-center py-4">
                        ...
                        <p class="text-muted mb-3"><span id="availableDate">-</span> a las <span id="availableTime">-</span></p>
